...
//            commonAlertView.removeFromSuperview()
    ...
    func setupAlert(alert title:String,alertDescription message:String? = nil,okAction:String,okColor:UIColor = .black,cancelAction:String? = nil, userImage:Data? = nil)
    {
       ...
       // alertView.messageDescriptionLbl.setFont(font: .medium(size: .BODY))
        ...
        alertView.userImageView.layer.cornerRadius = alertView.userImageView.height / 2
        alertView.userImageView.contentMode = .scaleAspectFill
        ...
        if let url = userImage  {
            alertView.userImageView.image = UIImage(data: url)
